...
import { isUrl } from '@components-ui/utils/validate'
...
import {
  ...
  mergeOptions,
  setStyleProperty
} from '@components-ui/utils/props'
...
export const useStyleProps = (params: IParams) => {
  ...
  const f = (
    options: object,
    notMerge = false,
    defaultProps: object = params.defaultStyleProps
  ) => {
    return filterStyleProperty(
      !notMerge ? mergeOptions(options, defaultProps) : options
    )
  }
  ...
  const v = (
    ...
    source: object = currentStyleProps.value
